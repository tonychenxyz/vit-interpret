...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None,
                 extract_last_k_th_token: int=-1, viz: bool = False):
        super().__init__()
        self.width = width
        self.layers = layers
        print('\n\n\n\n\ntransofmer total layers', layers)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        self.transformer = Transformer(width, layers, heads, extract_last_k_th_token=extract_last_k_th_token, viz=viz)
...
def build_model(state_dict: dict, extract_last_k_th_token, viz):
